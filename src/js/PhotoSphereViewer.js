...
import { ANGLE_THRESHOLD } from './data/constants';
...
import { bound, exitFullscreen, getShortestArc, isFullscreenEnabled, requestFullscreen, toggleClass } from './utils';
...
export class PhotoSphereViewer {
  ...
  constructor(options) {
     ...
     * @property {int} zoomLvl - current zoom level
     * @property {float} vFov - vertical FOV
     * @property {float} hFov - horizontal FOV
     * @property {float} aspect - viewer aspect ratio
     * @property {float} moveSpeed - move speed (computed with pixel ratio and configuration moveSpeed)
     * @property {int} gyroAlphaOffset - current alpha offset for gyroscope controls
     ...
     * @property {Promise} animationPromise - promise of the current animation (either go to position or image transition)
    ...
    /**
     * @summary Main viewer controller
     * @type {PSVRenderer}
     * @readonly
     */
    this.viewer = new PSVRenderer(this);

    /**
     * @summary Textures loaded
     * @type {PSVTextureLoader}
     * @readonly
     */
    ...
    this.once('render', () => {
      ...
      if (this.config.timeAnim !== false) {
        this.prop.startTimeout = window.setTimeout(() => this.startAutorotate(), this.config.timeAnim);
      }
      ...
      setTimeout(() => {
        // start render loop
        this.viewer.__renderLoop(+new Date());
   ...
   */
  isStereoEnabled() {
    return !!this.viewer.stereoEffect;
  }
  ...
  setPanorama(path, position, transition) {
    ...
    if (position) {
      this.dataHelper.cleanPosition(position);
    ...
    if (!transition || !this.config.transition || !this.scene) {
      ...
      this.prop.loadingPromise = this.textureLoader.__loadTexture(this.config.panorama)
        .then((texture) => {
          this.viewer.__setTexture(texture);
      ...
      this.prop.loadingPromise = this.textureLoader.__loadTexture(this.config.panorama)
        .then((texture) => {
          ...
          return this.viewer.transition(texture, position);
        })
        .then(done, done);
    }
  ...
  stopAutorotate() {
    if (this.prop.startTimeout) {
      window.clearTimeout(this.prop.startTimeout);
    ...
    if (SYSTEM.checkTHREE('DeviceOrientationControls')) {
      return SYSTEM.isDeviceOrientationSupported.then((supported) => {
        if (supported) {
          ...
          this.viewer.__startGyroscopeControl();
  ...
  stopGyroscopeControl() {
    if (this.isGyroscopeEnabled()) {
      this.viewer.__stopGyroscopeControl();
  ...
  startNoSleep() {
    ...
    if (!this.noSleep) {
      this.noSleep = new NoSleep(); // FIXME ?
    }
  ...
  rotate(position) {
     ...
     * @private
     */
    this.dataHelper.applyRanges(position).forEach(pos => this.trigger('_side-reached', pos));
  ...
  animate(position, speed) {
    ...
    const dLongitude = Math.abs(position.longitude - currentPosition.longitude);
    const dLatitude = Math.abs(position.latitude - currentPosition.latitude);
    ...
    const duration = this.dataHelper.speedToDuration(speed, position);

    // longitude offset for shortest arc
    const tOffset = getShortestArc(currentPosition.longitude, position.longitude);
    ...
    this.prop.animationPromise = new PSVAnimation({
      properties: {
        ...
        latitude : { start: currentPosition.latitude, end: position.latitude },
      },
      duration  : duration,
      easing    : 'inOutSine',
      onTick    : (newPosition) => {
