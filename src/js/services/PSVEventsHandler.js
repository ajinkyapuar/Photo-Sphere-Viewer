import { ACTIONS, DBLCLICK_DELAY, INERTIA_WINDOW, MOVE_THRESHOLD } from '../data/constants';
 ...
 */
export class PSVEventsHandler extends AbstractService {
  ...
  constructor(psv) {
     ...
     * @property {int} startMouseX - start x position of the click/touch
     * @property {int} startMouseY - start y position of the click/touch
     * @property {int} mouseX - current x position of the cursor
     * @property {int} mouseY - current y position of the cursor
     * @property {Array[]} mouseHistory - list of latest positions of the cursor, [time, x, y]
     * @property {int} pinchDist - distance between fingers when zooming
    ...
    if (this.config.mousemove) {
      this.psv.hud.container.style.cursor = 'move'; // FIXME => Hud
    }
    ...
    this.psv.on('_side-reached', (side) => {
      if (this.psv.isAutorotateEnabled()) {
        if (side === 'left' || side === 'right') {
          this.psv.viewer.__reverseAutorotate();
        }
      }
    });

    this.onResize = throttle(() => this.psv.autoSize(), 50);
  }
