...
import { clone, deepmerge, getAngle, parseAngle, parseSpeed } from '../utils';
 ...
 */
export class PSVDataHelper extends AbstractService {
  ...
  viewerCoordsToVector3(viewerPoint) {
    ...
    this.psv.viewer.raycaster.setFromCamera(screen, this.psv.viewer.camera);

    const intersects = this.psv.viewer.raycaster.intersectObjects(this.psv.viewer.scene.children);
  ...
  vector3ToViewerCoords(vector) {
    ...
    vectorClone.project(this.psv.viewer.camera);
  ...
  cleanPosition(position) {
    if ('x' in position && 'y' in position) {
      deepmerge(position, this.textureCoordsToSphericalCoords(position));
    }

    position.longitude = parseAngle(position.longitude);
    position.latitude = parseAngle(position.latitude, true);
  }
